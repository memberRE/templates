...
struct trie trietree[10000]; 
int triebody;

struct locate locatetree[10000];
int locatebody;
...
{
	struct trie*k=&trietree[triebody];
	triebody++;
	memset(k,0,sizeof(struct trie));
	return k;
} 

struct trie*locatememory()/*申请空内存*/
{
	struct locate*k=&locatetree[locatebody];
	locatebody++;
	memset(k,0,sizeof(struct locate));
	return k;
} 
